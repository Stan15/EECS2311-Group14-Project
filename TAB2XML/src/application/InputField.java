...
public class InputField {
	...
	private Tab fileSelectField = new Tab();
	...
	private void addFiles(List<File> fileList) {
		for (File file : fileList) {
			if (!this.files.contains(file.getAbsolutePath())) {
				if (this.files.size()>=this.maxFileCount) {return;}
	...
	private Button getFileSelectBttnAction() {
		...
		fileSelectBttn.setOnAction(new EventHandler<ActionEvent>() {
	...
	private HBox fileCard(String filePath) {
		...
		removeFileBttn.setOnAction(new EventHandler<ActionEvent>() {
			@Override
